defmodule Vagus.API.Tiers do
  ...
  @table [
    ...
    # -- no_security_check, minus what `Vagus.API.Auth` handles itself --------
    # `/supervisor/ping` is a literal member of upstream's `no_security_check`
    # (security.py L166) — served with no token at all. Vagus still requires
    # one (audit B1, deferred to this plan's phase 8); until that changes, the
    # route must at least not acquire a *role* on top of the token it should
    # not be asking for. The icon/logo GETs are the other `no_security_check`
    # members Vagus serves, and `Vagus.API.Auth.unauthenticated?/1` short-
    # circuits those before this table is ever consulted.
