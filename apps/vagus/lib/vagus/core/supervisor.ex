defmodule Vagus.Core.Supervisor do
  ...
  machinery (P3): `Vagus.Core.TokenStore`, a small `Finch` pool for
  outbound HTTP to Core, `Vagus.Core.Client` (lazy access-token exchange),
  and `Vagus.Core.EventPusher` (persistent WS client manager).
  ...
  (`Finch`, `Client`, `EventPusher`) — `EventPusher`'s `init/1` then
  re-subscribes and re-reads the refresh token from the fresh
  `TokenStore`, instead of being left permanently unsubscribed.
  """
